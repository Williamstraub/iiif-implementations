...
try:
    ...
    import simplejson as json # earlier


### This is a basic implementation of the IIIF Image API version 1.0
###     using the Python Image Library for manipulation functions
### Developer:  Rob Sanderson (azaroth42@gmail.com)
### Version: 1.0
### Date:  15 Jan 2013

BASEDIR = "/path/to/homedir"
BASEURL = "http://www.domain.com/"
PREFIX = "iiif"
FILEDIR = os.path.join(BASEDIR, '/path/to/images')

def parse_qs(data):
    # Python 2.4 doesn't have list comprehensions :(
    d = {}
    try:
        bits = data.split('&')
        for b in bits:
...
class ServiceHandler(WsgiApp):
    ...
    def __init__(self, d):
        ...
        # Search in FILEDIR for images
        fns = glob.glob(FILEDIR+'*jpg')
        fns.extend(glob.glob(FILEDIR+'*png'))
        fns.extend(glob.glob(FILEDIR+'*tif'))
...
<error xmlns="http://library.stanford.edu/iiif/image-api/ns/">
  ...
  <text>%s</text>
        ...
        self.status = status
        self.out_headers['Content-type'] = 'application/xml'
        return xml
    ...
    def make_info(self, infoId, image):
        ...
        <profile>%s</profile>
        </info>""" % (infoId, imageW, imageH, xmlformats, xmlqualities, profile)
    ...
    def handle(self):
        ...
        if bits:
            ...
            if self.idRe.match(identifier) == None:
                ...
                if self.badcharRe.match(identifier):
                    return self.xmlerror('identifier', 'Unescaped Characters', status=400)                
        ...
        if image == None:
            try:
                ...
                return self.xmlerror('identifier', 'Unsupported format for base image', status=501)
        ...
        if rotation != 0:
            ...
            try:
                ...
                # old version without expand. Math...
        ...
        try:
            ...
            return self.xmlerror('format', 'Unsupported format for format', status=501)
...
application = ServiceHandler('iiif')
